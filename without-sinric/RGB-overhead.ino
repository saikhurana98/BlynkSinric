...
//Pinout
  ...
  #define bright V5
  //Overhead Lights
  #define overheadSwitch V6
  #define overheadBright V7 
...
BLYNK_WRITE(overheadSwitch)
{
  int state = param.asInt();
  if (state == 1)
  {
    write_head(bright_head);
  }
  else
  {
    write_head(0);
...
BLYNK_WRITE(overheadBright) //Brightness slider widget
{ 
  bright_head = param.asInt();

  if (!bright_head){
    Blynk.virtualWrite(overheadSwitch,1);
  }else{
    Blynk.virtualWrite(overheadSwitch,1);
  }
...
{
  while (prev_head != brightness) 
  {
    if (prev_head < brightness)
    {
      prev_head+=inc;
    }
    else if (prev_head > brightness)
    {
      prev_head-=inc;
    }

    Serial.printf("Head: %d \n" , prev_head);
    analogWrite(headPin,prev_head * 4);
  }
}